...
stack_test_fr_Ca_CW_m = [];
stack_test_fr_Ca_CW_nm = [];
...
for c1ti = 1:length(cal.trial_info)
    ...
    if proceed == 1
        ...
        if abs(c1col-c2col) <=1 && c1col >= minlen && c2col >= minlen
            ...
            stim1 = trials(trialnum).direction_1;
            stim2 = trials(trialnum).direction_2;
...
%                 (contains(trials(trialnum).direction_2,'20')&contains(trials(trialnum).direction_1,'20'));
            ...
            tt = [];
            if strcmp(stim1(1:end-3),stim2(1:end-3)) && ~contains(stim1, 'CCW')
                tt = 1;
            elseif ~strcmp(stim1(1:end-3),stim2(1:end-3)) && ~contains(stim1, 'CCW')
                tt = 2;
            elseif ~strcmp(stim1(1:end-3),stim2(1:end-3)) && contains(stim1, 'CCW')
                tt = 3;
            elseif strcmp(stim1(1:end-3),stim2(1:end-3)) && contains(stim1, 'CCW')
