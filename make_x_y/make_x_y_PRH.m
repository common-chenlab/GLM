function [bigStruct, Y, X,  ROI_list_Ca1, ROI_list_Ca2, trial_info,checkvariable] = make_x_y_PRH(anm, datmat, directory, includeWhisker,denoise)
% David Lee
% Last update: 6/17/19
...
for c1ti = 1:length(cal.trial_info)
    ...
    if proceed == 1
        ...
        if abs(c1col-c2col) <=1 && c1col >= minlen && c2col >= minlen
            ...
            % Added by David - find trial type & result only using
            % trial_type_key and decision_key
            stimset = [stim1 ':' stim2];
            tt= [];
            tt = find(strcmp(trial_type_key,stimset));
